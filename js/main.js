(function () {
  ...
  /* ---------- длина контура карты и маршрута грузовика ----------
     у .map и .ln включён vector-effect:non-scaling-stroke, поэтому штрих живёт
     ...
     Для .ln (короткие сегменты маршрута) риск другой: без пересчёта дэш
     оказывается заметно длиннее реального сегмента, и вместо плавной
     прорисовки отрезок почти сразу выглядит нарисованным целиком. */
  function tuneMapLen() {
    Array.prototype.forEach.call(document.querySelectorAll('.art--map'), function (svg) {
      ...
      if (!vb || !w) return;
      var scale = w / vb;
      Array.prototype.forEach.call(svg.querySelectorAll('.map, .ln'), function (p) {
        var len;
